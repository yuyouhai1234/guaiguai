$(document).ready(function(){
	...
	var chapter=localStorage.getItem('num');
	var n=0;
	var m=0;
	$.ajax({
		type:"post",
...
function getCont(chapter,opera){
	$.ajax({
		...
		data:'{"km":"1","chapter":"'+chapter+'","direction":"next","operationid":"'+opera+'"}',
		dataType:'json',
		contentType:'application/json;charset=utf-8',
		success:function(data){
			//console.log(data)
	...
	function getDate(index,data){
		...
		var oOption1=$('<li class="option option-box"><span class="option-btn"/><span class="option-cont">'+data.ans1+'</span></span></li>');
		var oOption2=$('<li class="option option-box"><span class="option-btn"/><span class="option-cont">'+data.ans2+'</span></span></li>');
		var optionBox=$('<ul class="question-option"></ul>');
		optionBox.append(oOption1);
		optionBox.append(oOption2);
		if(data.ans3!=""){
			var oOption3=$('<li class="option option-box"><span class="option-btn"/><span class="option-cont">'+data.ans3+'</span></span></li>');
			optionBox.append(oOption3);
		}
		if(data.ans4!=''){
			var oOption4=$('<li class="option option-box"><span class="option-btn"/><span class="option-cont">'+data.ans4+'</span></span></li>');
